...
sicplot = IS2_plot(:,:,winter_mos,:);
sicplot = sicplot(~isnan(sicplot));

histogram(sicplot,sic_bins,'FaceColor',color_mapper(1,:),'FaceAlpha',0.5,'Normalization','pdf','edgecolor','none');
hold on

sicplot = IS2_plot_spec(:,:,winter_mos,:);
sicplot = sicplot(~isnan(sicplot));

histogram(sicplot,sic_bins,'FaceColor',color_mapper(2,:),'FaceAlpha',0.5,'Normalization','pdf','edgecolor','none');

sicplot = IS2_plot_weak(:,:,winter_mos,:);
sicplot = sicplot(~isnan(sicplot));

histogram(sicplot,sic_bins,'FaceColor',color_mapper(3,:),'FaceAlpha',0.5,'Normalization','pdf','edgecolor','none');

sicplot = IS2_plot_strong(:,:,winter_mos,:);
sicplot = sicplot(~isnan(sicplot));

histogram(sicplot,sic_bins,'FaceColor',color_mapper(4,:),'FaceAlpha',0.5,'Normalization','pdf','edgecolor','none');
...
sicplot = sicplot(~isnan(sicplot));
 ...
 histogram(sicplot,bias_bins,'FaceColor',sumcolor,'FaceAlpha',0.5,'Normalization','pdf','edgecolor','none');
    hold on

sicplot = IS2_plot_spec(:,:,winter_mos,:) - IS2_plot(:,:,winter_mos,:);
sicplot = sicplot(~isnan(sicplot));
histogram(sicplot,bias_bins,'FaceColor',wincolor,'FaceAlpha',0.5,'Normalization','pdf','edgecolor','none');
xlim([-.2 .2])
set(gca,'yticklabel','')

    grid on; box on;
    hold off
title('Specular - Dark','interpreter','latex')
